    ...
    <header id="topbar">
        ...
        <el-row>
            <el-col :span="6" class="topbar-left">
                <span style="font-size: 32px; font-weight: 600; color: white;margin-left: 15px;">在线考试后台管理系统</span>
                <el-button round size="mini" @click="toggleLeftComponentCollapse" style="margin: 25px;height:40%"><i
                        class="el-icon-s-unfold"></i></el-button>
            </el-col>
            <el-col :span="18" class="topbar-right">
                <i @click="toggle()"></i>
                <div class="user">
                    <span>{{ user.userName }}</span>
                    <img src="/static/img/admin.jpg" class="user-img" ref="img" @click="showSetting()" />
...
export default {
    ...
    methods: {
        toggleLeftComponentCollapse() {
            // 当点击按钮时触发父组件的toggleLeftComponentCollapse方法
            this.$emit('toggle-left-component-collapse');
        },
        // 管理员重置密码
        resetPsw() {
            if (this.oldPsw == "") {
            ...
            const encryptedPassword = md5(this.oldPsw);
            const newencryptedPassword = md5(this.newPsw)
            this.$axios(
                `/api/admin/resetPsw/${this.user.userId}/${encryptedPassword}/${newencryptedPassword}`
            ).then((res) => {
