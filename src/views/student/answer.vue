<template>
  <div id="answer">
    <!--顶部信息栏-->
    <div class="top">
      <ul class="item" style="display: flex;justify-content: space-between;">
        <li class="item-top">{{ examData.type }}-{{ examData.source }}</li>
        <li class="item-top">{{ userInfo.name }}-{{ userInfo.id }}</li>
      </ul>
    </div>
    <div class="flexarea">
      <!--左边题目编号区-->
      <transition name="slider-fade">
        <div class="left" v-if="slider_flag">
          ...
          mark: topic[3][index3].isMark == true,
            ...
            <el-button type="warning" style="width: 100%; font-size: 22px; margin: 25px 0 0 0" @click="finishExam()">
              结束考试</el-button>
          </div>
        </div>
      </transition>
      ...
      <transition name="slider-fade">
        <div class="right">
          <div class="title">
            ...
            <span style="">全卷共{{ topicCount[0] + topicCount[1] + topicCount[2] }}题
              <span v-if="!isPractice">倒计时：<b>{{ timeDisplay }}</b></span>
            </span>
          </div>
          <div class="content">
            <p class="topic">
            ...
            <div v-if="currentType == 1">
              ...
              <div class="analysis" v-if="isPractice && radio[index] != undefined">
                <ul>
                  <li>
                    <el-tag type="success">正确答案：</el-tag><span class="right">{{ reduceAnswer.rightAnswer }}</span>
                  </li>
          ...
          reduceAnswer.analysis == null ? "暂无解析" : reduceAnswer.analysis
            ...
            <div class="fill" v-if="currentType == 2">
              ...
              <div class="analysis" v-if="isPractice && fillAnswer[index][0] != undefined">
                <ul>
                  <li>
                    <el-tag type="success">正确答案：</el-tag><span class="right">{{ topic[2][index].answer }}</span>
                  </li>
                  <li><el-tag>题目解析：</el-tag></li>
                  <li>
                    {{
          ...
          <div class="operation">
            <ul class="end">
              <li @click="previous()" style="font-size: 24px" class="leftli">
                <span>
                  <=== 上一题</span>
              </li>
              <li @click="mark()" class="middleli">
                <span>标记</span>
              </li>
              <li @click="next()" style="font-size: 24px" class="rightli">
                <span>下一题 ===></span>
              </li>
            </ul>
          </div>
  ...
  methods: {
    ...
    getExamData() {
      ...
      this.$axios(`/api/exam/${examCode}`).then((res) => {
        ...
        this.index = 0;
        this.time = this.examData.totalTime * 60; //获取秒钟数
    ...
    showTime() {
      ...
      this.timer = setInterval(() => {
        // 更新分钟数和秒数
        const minutes = Math.floor(this.time / 60); // 分钟数
        const seconds = this.time % 60; // 秒数
        // 格式化分钟数和秒数
        const formattedMinutes = minutes < 10 ? `0${minutes}` : minutes;
        const formattedSeconds = seconds < 10 ? `0${seconds}` : seconds;
        // 更新倒计时显示
        this.timeDisplay = `${formattedMinutes}:${formattedSeconds}`;
